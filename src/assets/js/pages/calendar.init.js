...
(function(window, Calendar) {
    ...
    function setRenderRangeText() {
        ...
        var html = [];
        var date = moment(cal.getDate().getTime());
    
        if (viewName === 'day') {
            html.push(date.format('MMMM YYYY'));
        } else if (viewName === 'month' &&
            (!options.month.visibleWeeksCount || options.month.visibleWeeksCount > 4)) {
            html.push(date.format('MMMM YYYY'));
        } else {
            html.push(moment(cal.getDateRangeStart().getTime()).format('MMMM YYYY'));
            html.push(' ~ ');
            html.push(moment(cal.getDateRangeEnd().getTime()).format('MMMM YYYY'));
        }
        renderRange.innerHTML = html.join('');
    }

    function setRenderRangeText2() {
        var renderRange2 = document.getElementById('renderMonth');
        var options2 = cal.getOptions();
        var viewName2 = cal.getViewName();
        var html2 = [];
        var date2 = moment(cal.getDate().getTime());
    
        if (viewName2 === 'day') {
            html2.push(date2.format('MMMM'));
        } else if (viewName2 === 'month' &&
            (!options2.month.visibleWeeksCount || options2.month.visibleWeeksCount > 4)) {
            html2.push(date2.format('MMMM'));
        } else {
            html2.push(moment(cal.getDateRangeStart().getTime()).format('MMMM'));
            html2.push(' ~ ');
            html2.push(moment(cal.getDateRangeEnd().getTime()).format('MMMM'));
        }
        renderRange2.innerHTML = html2.join('');
    }
